                ...
                <td>
                  <div class="d-flex px-2 py-1">
                    <div class="form-check form-switch">
                      <input class="form-check-input" type="checkbox" role="switch" id="completado" data-id="${idFila}">
                    </div>
...
const idFila = btn.dataset.id;
 ...
 botonesEditar.forEach( btn => {
  const idFila = btn.dataset.id;
  btn.onclick = () => cargarEdicionFilas([...informacionTablas], idFila); 
 })
...
btnCompleto.forEach ( btn => {
  ...
  btn.addEventListener('change', () => {
    verificar([...informacionTablas], idFila, divPadre);
  } );
})
...
function verificar(tabla, idFila, divPadre) {
  const filaCompletada = divPadre.querySelector('.fila-completada');
  const filaSeleccionada = padreContenedor.querySelector(`tr[data-id='${idFila}']`);
  const tablaSeleccionada = filaSeleccionada.parentElement.parentElement.parentElement
                                            .parentElement.parentElement.parentElement;
  
// Obtenemos los indices de las tablas y filas 
 const indexTabla = tabla.findIndex( info => info.id == tablaSeleccionada.dataset.id);
// const indexFila = tabla[indexTabla].fila.findIndex( fil => fil.idFila == filaSeleccionada.dataset.id);

// console.log(indexTabla, indexFila)
  // tabla[indexTabla].fila.forEach(filas => {
    const imcompleto = divPadre.querySelector(`tr[data-id='${idFila}'] .imcompleto`);
    const checkbox = filaSeleccionada.querySelector(`input[type='checkbox']`);
...
function cargarEdicionFilas(tabla, idFila) {
  const filaSeleccionada = padreContenedor.querySelector(`tr[data-id='${idFila}']`);
  const tablaSeleccionada = filaSeleccionada.parentElement.parentElement.parentElement
                                            .parentElement.parentElement.parentElement;
  ...
  const filaEditar = tabla[indexTabla].fila.find( fil => fil.idFila == filaSeleccionada.dataset.id);
  
   
    // Editamos los valores de la fila específica
  tabla[indexTabla].fila[indexFila].nombreFila = filaEditar.nombreFila;
  tabla[indexTabla].fila[indexFila].idFila = filaEditar.idFila;
  idFilas = filaEditar.idFila;
    
    // Llenamos los formularios
    nombreFila.value = filaEditar.nombreFila;
    nombreTablaBuscar.value = tabla[indexTabla].nombreTabla;

  // Deshabilitamos el input de búsqueda
  nombreTablaBuscar.disabled = true;
  
  // Cambiamos el nombre dle boton
